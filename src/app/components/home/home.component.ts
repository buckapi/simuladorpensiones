...
export class HomeComponent implements OnInit {
  ...
  onSubmit() {
    if (this.cotizacionForm.valid) {
      ...
      const result = this.calculatePension(
        ...
        formValue.padres,
        parseInt(formValue.edadJubilacion, 10) // Convertir a número
      );
  ...
  // Función para obtener el porcentaje de edad
  getPorcentajeEdad(edadJubilacion: number): number {
    if (edadJubilacion >= 60 && edadJubilacion <= 65) {
      // Porcentaje aumenta gradualmente desde 80% a los 60 años hasta 100% a los 65 años
      return 80 + (edadJubilacion - 60) * 5;
    } else {
      return 0; // Si la edad está fuera del rango, no aplica pensión
    }
  }
  ...
  // Función para obtener el total de años reconocidos
